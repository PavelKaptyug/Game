...
Ammo::Ammo(int pos, const std::string& spritePath, SDL_Renderer* renderer, int direction)
...
void Shot(SDL_Renderer* renderer,  int pos,  int dis) {
    if (ammos.size() == 0) {
        ...
        Ammo ammo(pos, "sprites/bullet.jpg", renderer, spawnSide);
        ammo.loadSprite(renderer);
